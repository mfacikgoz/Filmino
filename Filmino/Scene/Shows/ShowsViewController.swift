...
extension ShowsViewController: UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    func collectionView (_ collectionView: UICollectionView,
                         numberOfItemsInSection section: Int) -> Int {
        viewModel?.shows.value.count ?? 0
    }
    
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        guard let viewModel else { return .init() }
        
        let data = viewModel.shows.value[indexPath.row]
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        viewModel?.didSelectShow(at: indexPath.row)
    }
    ...
    func collectionView(_ collectionView: UICollectionView, willDisplay cell: UICollectionViewCell, forItemAt indexPath: IndexPath) {
        ...
        if indexPath.row == viewModel.shows.value.count - 10 {
